...
table dist(i,j) distance between i & j
          1         2          3          4          5          6           7         8
1         0        990        2160       1060       500        2050        9999       2050
...
scalars
aa impact of delay time /10/
vcap capacity of vehicles /18/
m number of vehicle /3/
s time of disembarkation vehicles in cities / 15/  ;
variables
TC  ;
...
co5(i)$(ord(i)<>1)                     ..sum((j,k)$(ord(j)<>ord(i)),x(i,j,k))=e=1   ;
co6(p,k)                               ..sum(i,x(i,p,k))=e=sum(j,x(p,j,k)) ;
...
co11(i,j,k) $(ord(j)<>1)               ..t(i)+s+w(i)+DIST(i,j)=l=t(j)+10000*(1-x(i,j,k));
